...
exports.findEvenIndex = (arr) => {
  // Check each possible index in the array
  for (let i = 0; i < arr.length; i++) {
    // Calculate sum of left side (from start up to current index)
    const leftSum = arr.slice(0, i).reduce((sum, num) => sum + num, 0);

    // Calculate sum of right side (from next index to end)
    const rightSum = arr.slice(i + 1).reduce((sum, num) => sum + num, 0);

    // If both sides are equal, we found our answer!
    if (leftSum === rightSum) {
      return i;
    }
  }

  // If we checked all indexes and found nothing
  return -1;
};
