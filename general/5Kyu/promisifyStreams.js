 ...
 * 
 * Introduction
  * In this kata, you need to write a function to promisify a stream-like callback function.
 ...
 * Input/Output
  * Function promisify takes a callbackCaller function as the input. callbackCaller accepts a callback with one arguments. When data is received, callbackCaller will call this callback to transfer the data.

  * promisify should return an array with two elements. The first element is the data that the callback returned; the second element is the next promise which returns the next data with the same format.
 ...
 * Note
  ...
  * Callback in callbackCaller may not be called synchronously. Promise should be resolved immediately once the callback is called with no delay. Don't try to collect all data at one time.
 * 
 * @param {*} callbackCaller
 * @returns {Promise} A promise that resolves with [data, nextPromise]
 */

exports.promisify = (callbackCaller) => {
  let callbacks = [];
  let started = false;
  ...
  return new Promise((resolve) => {
    callbackCaller((data) => {
      if (!started) {
        started = true;
        // First data trigger
        const next = new Promise((res) => callbacks.push(res));
        resolve([data, next]);
      } else {
        // For every next data, resolve oldest pending promise
        const resolver = callbacks.shift();
        if (resolver) {
          const next = new Promise((res) => callbacks.push(res));
          resolver([data, next]);
        }
