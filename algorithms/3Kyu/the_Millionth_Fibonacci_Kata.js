...
exports.fib = (n) => {
  n = BigInt(n); // use BigInt for large values

  function fibPair(k) {
    if (k === 0n) return [0n, 1n];

    const [a, b] = fibPair(k >> 1n); // divide by 2
    const c = a * ((b << 1n) - a); // F(2k)
    const d = a * a + b * b; // F(2k+1)

    if (k & 1n) return [d, c + d]; // if k is odd
    else return [c, d]; // if k is even
  }
  ...
  if (n >= 0n) {
    ...
    const f = fibPair(-n)[0];
    return n % 2n === 0n ? -f : f; // apply negative rule
  }
};
