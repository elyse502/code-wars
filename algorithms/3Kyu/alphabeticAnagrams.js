...
exports.listPosition = (word) => {
  ...
  function getCount(letters) {
    ...
    let denom = 1;
    for (const k in freq) denom *= factorial(freq[k]);
    return factorial(letters.length) / denom;
  }
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    // count how many smaller letters can come before current
    const smallerLetters = remaining.filter((c) => c < current);

    for (const smaller of new Set(smallerLetters)) {
      const arr = [...remaining];
      arr.splice(arr.indexOf(smaller), 1);
      position += getCount(arr);
    }
  }
