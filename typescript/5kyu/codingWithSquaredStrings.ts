...
function buildSquare(text: string, n: number): string[][] {
    const square: string[][] = [];
    let index = 0;
...
function rotateClockwise(square: string[][]): string[][] {
...
function rotateCounterClockwise(square: string[][]): string[][] {
    const n = square.length;
    const rotated: string[][] = [];
...
function squareToString(square: string[][]): string {
    return square.map(row => row.join("")).join("\n");
}

function stringToSquare(s: string): string[][] {
    return s.split("\n").map(line => line.split(""));
}
...
export const code = (s: string): string => {
    ...
    const padded = s.padEnd(n * n, PAD);

    const square = buildSquare(padded, n);
    const rotated = rotateClockwise(square);

    return squareToString(rotated);
};

export const decode = (s: string): string => {
    if (s.length === 0) return "";

    const square = stringToSquare(s);
    const rotated = rotateCounterClockwise(square);

    const flat = rotated.map(row => row.join("")).join("");

    return flat.replace(new RegExp(PAD, "g"), "");
};